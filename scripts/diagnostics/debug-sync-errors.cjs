#!/usr/bin/env node
/**
 * Debug Sync Errors - Show full details for workouts that fail submission
 *
 * Runs the sync with verbose logging to identify why 36 workouts had errors.
 */
...
function parseWorkoutEvent(event) {
  ...
  if (distanceTag) {
    ...
    if (!isNaN(value)) {
      switch (unit) {
        case 'km': distanceMeters = value * 1000; break;
        case 'mi': distanceMeters = value * 1609.34; break;
        default: distanceMeters = value; break;
      }
    }
  }
...
async function submitWorkout(event) {
  ...
  try {
    ...
    try {
      ...
      result = { success: false, parseError: true, rawResponse: responseText.slice(0, 500) };
    }
    ...
    return {
      httpStatus: 0,
      httpOk: false,
      result: { success: false, networkError: error.message },
      workout,
      npub,
    };
  }
...
async function main() {
  console.log('=== Debug Sync Errors ===\n');
  console.log('Fetching events from Nostr...\n');
  ...
  const filter = { kinds: [1301], authors: SEASON_2_PUBKEYS, since: SEASON_2_START };
  ...
  for (const event of events) {
    const submitResult = await submitWorkout(event);
    const { httpStatus, httpOk, result, workout, npub } = submitResult;
    ...
    if (result.success) {
      ...
      flaggedCount++;
      console.log(`FLAGGED: ${event.id.slice(0, 12)} - ${result.reason}`);
    } else {
      // This is an error case - capture full details
      errors.push({
        eventId: event.id,
        pubkey: event.pubkey,
        npub,
  ...
  if (errors.length > 0) {
    ...
    for (const err of errors) {
      console.log('\n---');
      console.log(`Event ID: ${err.eventId}`);
      console.log(`Pubkey: ${err.pubkey.slice(0, 16)}...`);
      console.log(`npub: ${err.npub.slice(0, 20)}...`);
      console.log(`Created: ${err.created_at}`);
      console.log(`Activity: ${err.workout.activityType}`);
      console.log(`Distance: ${err.workout.distanceMeters ? (err.workout.distanceMeters / 1000).toFixed(2) + ' km' : 'null'}`);
      console.log(`Duration: ${err.workout.durationSeconds ? Math.round(err.workout.durationSeconds / 60) + ' min' : 'null'}`);
      console.log(`HTTP Status: ${err.httpStatus}`);
      console.log(`HTTP OK: ${err.httpOk}`);
      console.log(`Response: ${JSON.stringify(err.result, null, 2)}`);
    }
  }
