...
export async function startNativeWorkoutSession(
  ...
  if (!RUNSTRWorkoutBridge) {
    console.warn('[WorkoutSessionBridge] Native module not available - continuing without HKWorkoutSession');
    console.warn('[WorkoutSessionBridge] Note: Background tracking may be limited to 30 minutes on iOS');
    return;
  }

  try {
    await RUNSTRWorkoutBridge.startWorkoutSession(activityType);
    console.log(`[WorkoutSessionBridge] ✅ Started HKWorkoutSession for ${activityType}`);
    console.log('[WorkoutSessionBridge] iOS will now allow unlimited background location tracking');
  } catch (error) {
    console.error('[WorkoutSessionBridge] Failed to start HKWorkoutSession:', error);
    console.warn('[WorkoutSessionBridge] Continuing without workout session - 30 min limit may apply');
...
export async function stopNativeWorkoutSession(): Promise<void> {
  ...
  try {
    ...
    console.error('[WorkoutSessionBridge] Failed to stop HKWorkoutSession:', error);
